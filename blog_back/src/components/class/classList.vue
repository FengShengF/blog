        ...
        <el-table :data='tableData' style='width: 100%'>
            <el-table-column type='expand'>
                <template scope='props'>
                    <el-table :data='props.row.twoClass' style='width: 100%' :row-class-name='tableRowClassName'>
                        <el-table-column prop='cnname' label='中文标识' width='180'></el-table-column>
                        <el-table-column prop='enname' label='英文标识' width='180'></el-table-column>
            ...
            <el-table-column label='操作'>
                <template scope='scope'>
                    ...
                    <el-button size='small' @click='handleDelete(scope.$index,scope.row,"one")'>删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-dialog title='提示' :visible.sync='dialogVisible' size='tiny' :before-close='handleClose'>
            <span>您确定是否删除{{dialogMsg}}分类</span>
    ...
    methods:
    {
        ...
        handleEdit(index,data,state)
        {
            if(state=='one')
            {
                ...
                this.$router.push({path:"/back/amend_one_class",query:{state:'one'}})
            }
            else
            {
